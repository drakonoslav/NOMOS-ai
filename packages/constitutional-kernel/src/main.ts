 ...
 * ------------------------------------------------------------------
 *
 * Goal: move toward target position 1.0 while preserving energy/fuel.
 */
...
const conservationConstraints: ConstraintDefinition[] = [
  {
    ...
    evaluate: (x, _u, r) => {
      const bad = [...x, ...r].some((v) => !Number.isFinite(v)) ? 1 : 0;
      return bad;
    },
    tolerance: 1e-9,
  },
...
/**
 * One-step nominal roll-out helper.
 * Turns a proposed control sketch into a nominal trajectory summary
 * used to construct a CandidatePlan.
 * Resource depletion is coupled to |u| so plans cannot claim "free thrust".
 */
function simulateNominalPlan(
  x0: number[],
  r0: number[],
  controlSequence: number[][]
): { nominalX: number[]; nominalU: number[]; nominalR: number[] } {
  let x = [...x0];
  let r = [...r0];
  const tSec = measurement.timestamp / 1000;
  const dt = 1.0;

  for (const step of controlSequence) {
    x = modelRegistry.predictNextState(x, step, r, disturbancesAtObserve, tSec, dt);
    const fuelBurn = 0.08 * Math.abs(step[0] ?? 0) + 0.01;
    r = [Math.max(0, (r[0] ?? 0) - fuelBurn)];
  }

  const meanU =
    controlSequence.length > 0
      ? controlSequence.reduce((sum, s) => sum + (s[0] ?? 0), 0) / controlSequence.length
      : 0;

  return { nominalX: x, nominalU: [meanU], nominalR: r };
}

/**
 * ------------------------------------------------------------------
 * 8) MISSION CONTEXT + LLM PROPOSER
 * ------------------------------------------------------------------
 *
 * The LLM proposer generates candidate plans.
 * It may propose — it may not certify, verify, or actuate.
 * All screening happens downstream in the constitutional chain.
 */

const missionContext: MissionContext = {
  missionId: "tiny-mock-mission",
  objectiveText:
    "Move position toward 1.0 while preserving energy, fuel, and bounded lawful operation.",
  currentTime: measurement.timestamp,
  horizonSteps: 4,
  controlDim: 1,
  stateDim: 2,
...
console.log(`[llm_proposer] generated proposals: ${proposalBundle.proposals.length}`);
if (proposalBundle.reasons.length > 0) {
  console.log("[llm_proposer] notes:");
  for (const reason of proposalBundle.reasons) {
    console.log(`  - ${reason}`);
  }
}
...
/**
 * ------------------------------------------------------------------
 * 9) TURN PROPOSALS INTO CANDIDATE PLANS
 * ------------------------------------------------------------------
 *
 * Each CONTROL_PLAN proposal is:
 *   1. simulated into a nominal trajectory (resource-coupled)
 *   2. wrapped in a FeasibilityInput from that trajectory
 *   3. converted to CandidatePlan via proposer.toCandidatePlan()
 *
 * Sensitivity scaling is proportional to max control magnitude:
 * larger actuation → larger sensitivity matrix → lower robustness radius.
 * This means aggressive proposals are penalised constitutionally.
 *
 * No plan is lawful at this stage — screening happens downstream.
 */

const controlProposals = proposalBundle.proposals.filter(
  (p): p is LLMProposal => p.kind === "CONTROL_PLAN" && !!p.planSketch
);

const candidatePlans: CandidatePlan[] = controlProposals.map((proposal) => {
  ...
  const feasibilityInput = makeFeasibilityInput(
    nominal.nominalX,
    nominal.nominalU,
    nominal.nominalR
  );

  const maxControl = Math.max(...controlSequence.map((s) => Math.abs(s[0] ?? 0)), 0);
  const sensitivityScale = 1 + maxControl * 6;
  ...
  return proposer.toCandidatePlan({
    ...
    robustnessConfig: {
      ...
      sensitivityMatrix: [
        [1.1 * sensitivityScale, 0.1],
        [0.05, 1.0 + 0.5 * sensitivityScale],
      ],
    },
  });
});

console.log(`[planner] candidate plans from proposer: ${candidatePlans.length}`);
...
console.log(`LLM proposals             : ${proposalBundle.proposals.length}`);
